...
// #include "../src_mpkuse/utils/ElapsedTime.h"
...
void LCDetector::process(const unsigned image_id,
  ...
  // std::cout << "\t" << timer.toc_milli() << " Storing the keypoints and descriptors (ms):\n";

  // Adding the current image to the queue to be added in the future
  // timer.tic();
  queue_ids_.push(image_id);
  // std::cout << "\t" << timer.toc() << " Adding the current image to the queue to be added in the future (ms)\n";
  ...
  // std::cout << "\t" << timer.toc_milli() << " Adding new hypothesis(ms)\n";
  ...
  // std::cout << "\t" << timer.toc_milli() << " Searching the query descriptors against the features (ms):\n";

  // Filtering matches according to the ratio test
  // timer.tic();
  std::vector<cv::DMatch> matches;
  filterMatches(matches_feats, &matches);
  // std::cout << "\t" << timer.toc_milli() << " Filtering matches according to the ratio test (ms)\n";
  ...
  // std::cout << "\t" << timer.toc_milli() << " We look for similar images according to the filtered matches found\n";
  ...
  // std::cout << "\t" << timer.toc_milli() << " Filtering the resulting image matchings\n";

  // timer.tic();
  std::vector<Island> islands;
  buildIslands(image_matches_filt, &islands);
  // std::cout << "\t" << timer.toc_milli() << " buildIslands()\n";
...
void LCDetector::addImage(const unsigned image_id,
  ...
  if (index_->numImages() == 0) {
    ...
    // Searching the query descriptors against the features
    index_->searchDescriptors(descs, &matches_feats, 2, 64);
    // std::cout << "\t\t" << timer.toc_milli() << " [LCDetector::addImage]Searching the query descriptors against the features\n";

    // Filtering matches according to the ratio test
    // timer.tic();
    std::vector<cv::DMatch> matches;
    filterMatches(matches_feats, &matches);
    // std::cout << "\t\t" << timer.toc_milli() << " [LCDetector::addImage]Filtering matches according to the ratio test\n";

    // Finally, we add the image taking into account the correct matchings
    // timer.tic();
    index_->addImage(image_id, kps, descs, matches);
    // std::cout << "\t\t" << timer.toc_milli() << " [LCDetector::addImage]Finally, we add the image taking into account the correct matchings\n";
  }
}
